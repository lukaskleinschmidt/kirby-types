...
class BlockMethod extends Method
{
    ...
    public function document(string $type, ?string $block = null): static
    {
        $comment = $this->comment();

        $comment->description = $block
            ? 'Uses a `' . $type . '` field in the `' . $block . '` block.'
            : 'Uses a `' . $type . '` field.';
    ...
    public function merge(BlockMethod $method): static
    {
        $comment = $this->comment();
